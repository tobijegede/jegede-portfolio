...
In trying to maintain the same objective as the original visualization, namely determining if public transit ridership has **rebounded**, so it may be good to use the first measure (Jan 2020 as the marker of pre-pandemic numbers) and see the percentage change in ridership *relative* to that particular value. This could also enable both of the visualizations to be represented in the same graph.
