...
- In response to the feedback from reviewers two and three on having relative percent change information, I decided to alter my visualization to examine percentage change over time. This would allow me to include all cities in one graph instead of two. In trying to maintain the same objective as the original visualizations, namely determining if public transit ridership has **rebounded**, the percentage change values will be calcualted relative to Jan 2020, which is used as the "pre-pandemic" norm. 
